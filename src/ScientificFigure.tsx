...
import { Braces, ChartScatter, CircleAlert, Dna, Eye, Grid3x3, MousePointer2, Save } from "lucide-react";
...
function buildScatterTemplatePoints(): ScatterPoint[] {
  const clusters = [
    { category: "Neural", x: -2.7, y: -1.1, scaleX: 1.25, scaleY: .8, count: 28 },
    { category: "Immune", x: 2.55, y: 1.15, scaleX: .75, scaleY: .62, count: 18 },
    { category: "Stromal", x: .2, y: 1.75, scaleX: 1.05, scaleY: .55, count: 20 },
    { category: "Epithelial", x: .65, y: -.55, scaleX: .82, scaleY: .68, count: 18 },
    { category: "Cycling", x: 2.05, y: -1.65, scaleX: .72, scaleY: .45, count: 14 },
    { category: "Vascular", x: -1.25, y: 1.55, scaleX: .5, scaleY: .43, count: 12 },
  ];
  let pointIndex = 0;
...
export const figureTemplates: Record<ScientificFigureData["kind"], string> = {
  scatter: JSON.stringify({
    ...
    subtitle: "Synthetic local template, replace with measured data",
    x_label: "UMAP 1",
    y_label: "UMAP 2",
  ...
  matrix: JSON.stringify({
    version: 1,
    kind: "matrix",
    title: "Quantitative matrix",
    subtitle: "Size and tone encode independent values",
  ...
  sequence: JSON.stringify({
    ...
    subtitle: "Synthetic architecture with exact feature coordinates",
    length: 980,
    features: [
  ...
  return (
    <figure className="scientific-figure">
      ...
      <div className="scientific-canvas">
        <svg viewBox="0 0 760 440" role="img" aria-labelledby={titleId}>{content}</svg>
      </div>
    </figure>
  );
}
...
function ScatterFigure({ figure, selectedKey, onSelect }: { figure: ScientificFigureData; selectedKey: string; onSelect?: (selector: FigureSelector) => void }) {
  ...
  const y = (value: number) => 354 - ((value - yMin) / yRange) * 300;
  return <>
    {[0, 1, 2, 3, 4].map(index => <g key={index}><line className="figure-grid" x1={72} x2={702} y1={54 + index * 75} y2={54 + index * 75} /><line className="figure-grid" y1={54} y2={354} x1={72 + index * 157.5} x2={72 + index * 157.5} /></g>)}
    <line className="figure-axis" x1="72" x2="702" y1="354" y2="354" /><line className="figure-axis" x1="72" x2="72" y1="54" y2="354" />
    <text className="figure-axis-label" x="387" y="410" textAnchor="middle">{figure.x_label}</text>
    <text className="figure-axis-label" x="20" y="204" textAnchor="middle" transform="rotate(-90 20 204)">{figure.y_label}</text>
    {points.map(point => {
      const selector: FigureSelector = { kind: "point", id: point.id };
      const selected = selectedKey === selectorKey(selector);
      return <circle key={point.id} className={selected ? "figure-mark selected" : "figure-mark"} cx={x(point.x)} cy={y(point.y)} r={selected ? 6 : 3.5} fill={colors.get(point.category)} tabIndex={0} role="button" aria-label={`${point.label}, ${point.category}, x ${point.x}, y ${point.y}`} data-selector={JSON.stringify(selector)} onClick={() => onSelect?.(selector)} onKeyDown={event => activate(event, () => onSelect?.(selector))}><title>{point.label}</title></circle>;
    })}
    {[...colors].map(([category, color], index) => <g key={category} transform={`translate(${86 + index * 124} 386)`}><circle r="4" fill={color} /><text className="figure-legend" x="10" y="4">{category}</text></g>)}
  </>;
}
...
function MatrixFigure({ figure, selectedKey, onSelect }: { figure: ScientificFigureData; selectedKey: string; onSelect?: (selector: FigureSelector) => void }) {
  ...
  const width = 580 / columns.length, height = 290 / rows.length;
  return <>
    {rows.map((row, index) => <text key={row} className="figure-tick" x="145" y={80 + index * height + height / 2 + 4} textAnchor="end">{row}</text>)}
    {columns.map((column, index) => <text key={column} className="figure-tick" x={160 + index * width + width / 2} y="55" textAnchor="middle">{column}</text>)}
    {rows.flatMap((_, row) => columns.map((__, column) => <rect key={`${row}:${column}`} className="figure-matrix-cell" x={160 + column * width} y={80 + row * height} width={width} height={height} />))}
    {cells.map(cell => {
      ...
      return <circle key={`${cell.row}:${cell.column}`} className={selected ? "figure-matrix-mark selected" : "figure-matrix-mark"} cx={160 + cell.column * width + width / 2} cy={80 + cell.row * height + height / 2} r={selected ? radius + 2 : radius} fillOpacity={0.25 + ((cell.value - valueMin) / valueRange) * 0.75} tabIndex={0} role="button" aria-label={`${rows[cell.row]}, ${columns[cell.column]}, value ${cell.value}, size ${cell.size}`} data-selector={JSON.stringify(selector)} onClick={() => onSelect?.(selector)} onKeyDown={event => activate(event, () => onSelect?.(selector))}><title>{`${rows[cell.row]} / ${columns[cell.column]}: ${cell.value}`}</title></circle>;
    })}
    <text className="figure-note" x="160" y="405">Circle size and opacity represent the supplied values.</text>
  </>;
}
...
function SequenceFigure({ figure, selectedKey, onSelect }: { figure: ScientificFigureData; selectedKey: string; onSelect?: (selector: FigureSelector) => void }) {
  ...
  return <>
    <line className="figure-sequence-line" x1="70" x2="700" y1="220" y2="220" />
    {[0, .25, .5, .75, 1].map(fraction => <g key={fraction}><line className="figure-axis" x1={70 + fraction * 630} x2={70 + fraction * 630} y1="211" y2="229" /><text className="figure-tick" x={70 + fraction * 630} y="250" textAnchor="middle">{Math.round(1 + fraction * (length - 1))}</text></g>)}
    {features.map((feature, index) => {
      const selector: FigureSelector = { kind: "feature", id: feature.id };
      const selected = selectedKey === selectorKey(selector);
      const start = x(feature.start), end = x(feature.end), lane = index % 4, y = 106 + lane * 58;
    ...
    {[...colors].map(([category, color], index) => <g key={category} transform={`translate(${82 + index * 130} 330)`}><rect width="12" height="8" rx="2" fill={color} /><text className="figure-legend" x="19" y="8">{category}</text></g>)}
  </>;
}
...
export function ScientificFigureWorkbench({ projects, language }: { projects: ProjectOption[] | null; language: Language }) {
  ...
  return <section className="figure-workbench">
    <div className="figure-preview-pane">
      {preview ? <>
        <ScientificFigureView figure={preview.figure} selected={selected} onSelect={setSelected} />
        <div className="figure-provenance"><span>{preview.renderer_id}/v{preview.renderer_version}</span><span>SHA-256 {preview.source_sha256.slice(0, 12)}</span><span>{preview.execution === "disabled" ? (language === "fr" ? "Aucune exécution" : "No execution") : preview.execution}</span></div>
        <div className="figure-selection"><MousePointer2 size={15} /><div><span>{language === "fr" ? "Sélecteur exact" : "Exact selector"}</span><code>{selected ? JSON.stringify(selected) : (language === "fr" ? "Sélectionnez une marque" : "Select a mark")}</code></div></div>
    ...
    <aside className="figure-editor" aria-label={language === "fr" ? "Inspecteur de figure" : "Figure inspector"}>
      <div className="figure-editor-heading"><Braces size={16} /><div><h3>{language === "fr" ? "Données de la figure" : "Figure data"}</h3><p>JSON v1, local</p></div></div>
      <form onSubmit={event => void renderFigure(event)}>
        <label>{language === "fr" ? "Type de figure" : "Figure type"}<select value={kind} onChange={event => chooseKind(event.target.value as ScientificFigureData["kind"])}><option value="scatter">Scatter embedding</option><option value="matrix">Quantitative matrix</option><option value="sequence">Sequence track</option></select></label>
        <label>{language === "fr" ? "Spécification JSON" : "JSON specification"}<textarea value={source} onChange={event => { setSource(event.target.value); setPreview(null); setStatus(null); }} spellCheck={false} /></label>
        <button className="action" type="submit" disabled={busy}><Eye size={14} />{language === "fr" ? "Valider et afficher" : "Validate and render"}</button>
      </form>
