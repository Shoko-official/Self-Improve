...
function RagSurface({ language }: { language: Language }) {
  const [uri, setUri] = useState(""); const [label, setLabel] = useState(""); const [content, setContent] = useState(""); const [query, setQuery] = useState(""); const [cases, setCases] = useState('[{"query":"","expected_sources":[]}]'); const [metrics, setMetrics] = useState<{ cases: number; hits: number; recall_at_k: number } | null>(null); const [citations, setCitations] = useState<Array<{ source_label: string; source_uri: string; text: string; start_offset: number; end_offset: number }> | null>(null); const [error, setError] = useState<string | null>(null);
