...
import { operationalText, surfaceText, type Language } from "./i18n";
  ...
  return (
    <section className="science-workbench">
      ...
      <div className="science-view">
        ...
        {panel === "evidence" && <div className="science-evidence-view"><ScienceEvidenceSurface language={language} /><ReviewerSurface /></div>}
        {panel === "artifacts" && <div className="science-evidence-view"><ArtifactWorkspaceSurface language={language} /></div>}
      </div>
    </section>
  );
}
...
function ArtifactWorkspaceSurface({ language }: { language: Language }) {
  const [inspectedVersionId, setInspectedVersionId] = useState<string | undefined>();
  return <><ArtifactsSurface language={language} onInspectVersion={setInspectedVersionId} /><RagSurface language={language} /><AnnotationSurface language={language} initialVersionId={inspectedVersionId} /></>;
}

export function RagSurface({ language }: { language: Language }) {
  const [uri, setUri] = useState(""); const [label, setLabel] = useState(""); const [content, setContent] = useState(""); const [selectedFile, setSelectedFile] = useState<string | null>(null); const [query, setQuery] = useState(""); const [cases, setCases] = useState('[{"query":"","expected_sources":[]}]'); const [metrics, setMetrics] = useState<{ cases: number; hits: number; recall_at_k: number } | null>(null); const [citations, setCitations] = useState<Array<{ source_label: string; source_uri: string; text: string; start_offset: number; end_offset: number }> | null>(null); const [catalog, setCatalog] = useState<LocalModelCatalog | null>(null); const [embeddingModel, setEmbeddingModel] = useState(""); const [error, setError] = useState<string | null>(null);
  async function refreshEmbeddingModels() { try { setError(null); setCatalog(await invoke<LocalModelCatalog>("local_model_catalog_development")); } catch (reason) { setError(reason instanceof Error ? reason.message : "FR-RAG-EMBEDDING-CATALOG"); } }
  useEffect(() => { void refreshEmbeddingModels(); }, []);
  const embeddingArgs = () => ({ embeddingModel: embeddingModel || null });
  async function ingest(event: FormEvent) { event.preventDefault(); try { setError(null); await invoke("rag_ingest_development", { sourceUri: uri, sourceLabel: label, content, ...embeddingArgs() }); setContent(""); } catch (reason) { setError(reason instanceof Error ? reason.message : "FR-RAG-INGEST"); } }
  async function search(event: FormEvent) { event.preventDefault(); try { setError(null); setCitations((await invoke<{ citations: Array<{ source_label: string; source_uri: string; text: string; start_offset: number; end_offset: number }> }>("rag_search_development", { query, ...embeddingArgs() })).citations); } catch (reason) { setError(reason instanceof Error ? reason.message : "FR-RAG-SEARCH"); } }
  async function evaluate(event: FormEvent) { event.preventDefault(); try { setError(null); setMetrics(await invoke("rag_evaluate_development", { casesJson: cases, ...embeddingArgs() })); } catch (reason) { setError(reason instanceof Error ? reason.message : "FR-RAG-EVALUATE"); } }
  async function selectLocalFile(file: File | undefined) {
    if (!file) return;
    const supported = /\.(txt|md|markdown|csv|tsv|json|jsonl|py|r|log)$/i.test(file.name);
    if (!supported) { setError(language === "fr" ? "Choisissez un fichier texte pris en charge." : "Choose a supported text file."); return; }
    if (file.size > 1_000_000) { setError(language === "fr" ? "Le fichier depasse la limite locale de 1 Mo." : "The file exceeds the 1 MB local limit."); return; }
    try { setError(null); const text = await file.text(); setUri(`local://selected/${encodeURIComponent(file.name)}`); setLabel(file.name); setContent(text); setSelectedFile(file.name); }
    catch { setError(language === "fr" ? "Le fichier local n'a pas pu etre lu." : "The local file could not be read."); }
  }
  const ollamaModels = catalog?.ollama.models ?? [];
  return <section className="surface"><div className="surface-mark">LOCAL RAG</div><h2>{language === "fr" ? "Sources et citations" : "Sources and citations"}</h2><p>{language === "fr" ? "Les sources restent locales. Chaque résultat conserve son URI et ses offsets exacts." : "Sources stay local. Every result preserves its URI and exact offsets."}</p><div className="project-form"><label htmlFor="rag-embedding-model">{language === "fr" ? "Embeddings Ollama locaux" : "Local Ollama embeddings"}</label><select id="rag-embedding-model" value={embeddingModel} onChange={event => setEmbeddingModel(event.target.value)}><option value="">{language === "fr" ? "Lexical uniquement" : "Lexical only"}</option>{ollamaModels.map(model => <option key={model} value={model}>{model}</option>)}</select><p>{embeddingModel ? (language === "fr" ? "Le modèle est vérifié par Ollama pendant l'opération. Réindexez une source pour y ajouter ses vecteurs." : "The model is verified by Ollama during the operation. Reingest a source to add its vectors.") : (language === "fr" ? "Aucun modèle d'embedding sélectionné. La recherche reste lexicale." : "No embedding model is selected. Search remains lexical.")}</p><button className="minor-action" type="button" onClick={() => void refreshEmbeddingModels()}>{language === "fr" ? "Actualiser les modèles" : "Refresh models"}</button></div><form className="project-form" onSubmit={event => void ingest(event)}><label>{language === "fr" ? "Ajouter un fichier local" : "Add a local file"}<input type="file" accept=".txt,.md,.markdown,.csv,.tsv,.json,.jsonl,.py,.r,.log,text/plain,text/markdown,text/csv,application/json" onChange={event => void selectLocalFile(event.target.files?.[0])} /></label>{selectedFile && <p className="rag-file-note"><FileStack size={14} />{selectedFile}</p>}<input value={uri} onChange={event => setUri(event.target.value)} placeholder="file:///notes/source.md" required /><input value={label} onChange={event => setLabel(event.target.value)} placeholder={language === "fr" ? "Nom de source" : "Source label"} required /><textarea value={content} onChange={event => setContent(event.target.value)} placeholder={language === "fr" ? "Texte local à indexer" : "Local text to index"} required /><button className="minor-action" type="submit">{language === "fr" ? "Indexer" : "Ingest"}</button></form><form className="project-form" onSubmit={event => void search(event)}><input value={query} onChange={event => setQuery(event.target.value)} placeholder={language === "fr" ? "Rechercher" : "Search"} required /><button className="action" type="submit">{language === "fr" ? "Rechercher les citations" : "Search citations"}</button></form><form className="project-form" onSubmit={event => void evaluate(event)}><textarea value={cases} onChange={event => setCases(event.target.value)} aria-label="Held-out cases JSON" required /><button className="minor-action" type="submit">{language === "fr" ? "Évaluer le held-out" : "Evaluate held-out"}</button></form>{error && <p className="agent-error">{error}</p>}{metrics && <Evidence rows={[`${metrics.hits}/${metrics.cases} recall@k: ${metrics.recall_at_k}`]} />}{citations && <Evidence rows={citations.length ? citations.map(citation => `${citation.source_label} [${citation.start_offset}-${citation.end_offset}] ${citation.text}`) : [language === "fr" ? "Aucune citation" : "No citations"]} />}</section>;
}

export function AnnotationSurface({ language, initialVersionId }: { language: Language; initialVersionId?: string }) {
  const [versionId, setVersionId] = useState(initialVersionId ?? ""); const [targetKind, setTargetKind] = useState("text"); const [selector, setSelector] = useState('{"offset":0}'); const [body, setBody] = useState("");
  const [annotations, setAnnotations] = useState<Array<{ id: string; target_kind: string; body: string; consumed_at: string | null }> | null>(null); const [error, setError] = useState<string | null>(null);
  const copy = language === "fr" ? { title: "Commentaires exacts sur un artefact", description: "Chaque annotation reste attachée à la version d'artefact indiquée. Une version absente est refusée.", label: "Version de l'artefact, cible, sélecteur et note", save: "Enregistrer l'annotation", refresh: "Recharger les annotations", kinds: { text: "Texte", code: "Code", markdown: "Markdown", latex: "LaTeX", pdf_region: "Région PDF", image_point: "Point d'image", html_element: "Élément HTML", transcript_region: "Région de transcription" } } : { title: "Exact artifact feedback", description: "Annotations stay attached to the artifact version you name. A missing version is rejected.", label: "Artifact version, target, selector, and note", save: "Save annotation", refresh: "Refresh annotations", kinds: { text: "Text", code: "Code", markdown: "Markdown", latex: "LaTeX", pdf_region: "PDF region", image_point: "Image point", html_element: "HTML element", transcript_region: "Transcript region" } };
  useEffect(() => { if (initialVersionId) setVersionId(initialVersionId); }, [initialVersionId]);
  async function load() { try { setError(null); setAnnotations((await invoke<{ annotations: Array<{ id: string; target_kind: string; body: string; consumed_at: string | null }> }>("project_annotations_development", { artifactVersionId: versionId })).annotations); } catch (reason) { setError(reason instanceof Error ? reason.message : surfaceText(language, "annotation")); } }
  async function create(event: FormEvent) { event.preventDefault(); try { await invoke("create_project_annotation_development", { artifactVersionId: versionId, targetKind, selector, body }); setBody(""); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : surfaceText(language, "annotation")); } }
  return <section className="surface annotation-panel"><div className="surface-mark">{language === "fr" ? "ANNOTATIONS VERSIONNÉES" : "VERSIONED ANNOTATIONS"}</div><h2>{copy.title}</h2><p>{copy.description}</p><form className="project-form" onSubmit={event => void create(event)}><label htmlFor="annotation-version">{copy.label}</label><input id="annotation-version" value={versionId} onChange={event => setVersionId(event.target.value)} placeholder={surfaceText(language, "artifactVersionId")} required /><select value={targetKind} onChange={event => setTargetKind(event.target.value)}>{Object.entries(copy.kinds).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><input value={selector} onChange={event => setSelector(event.target.value)} placeholder='{"page":2}' required /><textarea value={body} onChange={event => setBody(event.target.value)} placeholder={surfaceText(language, "annotation")} required /><button className="action" type="submit">{copy.save}</button></form>{error && <p className="agent-error">{error}</p>}<button className="minor-action" onClick={() => void load()}>{copy.refresh}</button>{annotations && <Evidence rows={annotations.length ? annotations.map(annotation => `${copy.kinds[annotation.target_kind as keyof typeof copy.kinds] ?? annotation.target_kind}: ${annotation.body}`) : [surfaceText(language, "noOpenAnnotations")]} />}</section>;
}
...
function ScienceEvidenceSurface({ language }: { language: Language }) {
  const [queries, setQueries] = useState<Array<{ id: string; query_text: string; source: string; result_count: number; accessed_at: string }> | null>(null);
  const [claims, setClaims] = useState<ClaimRecord[] | null>(null);
  const [query, setQuery] = useState("");
  const [source, setSource] = useState("");
  const [count, setCount] = useState("0");
  const [claimType, setClaimType] = useState("observation");
  ...
  const copy = language === "fr" ? {
    kicker: "RECHERCHE LOCALE", title: "Carnet de preuves", description: "Conservez ce que vous avez recherché et ce qui soutient une conclusion. Les connecteurs de littérature distants ne sont pas configurés sur cette machine.", searches: "Recherches enregistrées", claims: "Affirmations suivies", recordSearch: "Consigner une recherche", query: "Requête exacte", source: "Source consultée", results: "Résultats observés", save: "Enregistrer", refresh: "Actualiser", addClaim: "Ajouter une affirmation", type: "Nature", claim: "Affirmation", uncertainty: "Incertitude ou limite", evidence: "Référence de preuve", selector: "Zone précise", saveClaim: "Enregistrer l'affirmation", noSearch: "Aucune recherche enregistrée", noClaim: "Aucune affirmation enregistrée", detail: "Les détails techniques de preuve", supported: "Étayer", disputed: "Contester", retracted: "Retirer"
  } : {
    kicker: "LOCAL RESEARCH", title: "Evidence desk", description: "Keep a record of what you searched and what supports a conclusion. Remote literature connectors are not configured on this machine.", searches: "Recorded searches", claims: "Tracked claims", recordSearch: "Record a search", query: "Exact query", source: "Source consulted", results: "Observed results", save: "Save record", refresh: "Refresh", addClaim: "Add a claim", type: "Kind", claim: "Claim", uncertainty: "Uncertainty or limitation", evidence: "Evidence reference", selector: "Exact location", saveClaim: "Save claim", noSearch: "No searches recorded", noClaim: "No claims recorded", detail: "Technical evidence details", supported: "Mark supported", disputed: "Mark disputed", retracted: "Retract"
  };
  async function refresh() {
    try {
      setError(null);
      const [queryResult, claimResult] = await Promise.all([
        invoke<{ queries: Array<{ id: string; query_text: string; source: string; result_count: number; accessed_at: string }> }>("literature_queries_development"),
        invoke<{ claims: ClaimRecord[] }>("scientific_claims_development"),
      ]);
      setQueries(queryResult.queries);
      setClaims(claimResult.claims);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Science records are unavailable."); }
  }
  async function submitSearch(event: FormEvent) {
    event.preventDefault();
    try {
      await invoke("record_literature_query_development", { query, source, resultCount: Number(count) });
      setQuery(""); setSource(""); setCount("0"); await refresh();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Literature record failed."); }
  }
  async function submitClaim(event: FormEvent) {
    event.preventDefault();
    try {
      await invoke("create_scientific_claim_development", { claimType, claimText, uncertainty, evidenceUri, evidenceSelector });
      setClaimText(""); setUncertainty(""); setEvidenceUri(""); setEvidenceSelector(""); await refresh();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Claim record failed."); }
  }
  async function setClaimStatus(claimId: string, claimStatus: ClaimRecord["status"]) {
    try { await invoke("set_scientific_claim_status_development", { claimId, claimStatus }); await refresh(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Claim update failed."); }
  }
  useEffect(() => { void refresh(); }, []);
  return <section className="surface science-evidence-desk">
    <header className="science-evidence-header"><div><div className="surface-mark">{copy.kicker}</div><h2>{copy.title}</h2><p>{copy.description}</p></div><button className="minor-action" type="button" onClick={() => void refresh()}><RefreshCw size={14} />{copy.refresh}</button></header>
    {error && <p className="agent-error">{error}</p>}
    <div className="science-evidence-counts"><div><span>{copy.searches}</span><strong>{queries?.length ?? "-"}</strong></div><div><span>{copy.claims}</span><strong>{claims?.length ?? "-"}</strong></div></div>
    <section className="science-evidence-section"><div className="science-section-heading"><div><h3>{copy.recordSearch}</h3><p>{language === "fr" ? "Saisissez la recherche telle qu'elle a été exécutée et sa source." : "Enter the search exactly as it was run, with its source."}</p></div></div><form className="science-compact-form" onSubmit={event => void submitSearch(event)}><label>{copy.query}<input value={query} onChange={event => setQuery(event.target.value)} placeholder="single-cell quality control" required /></label><label>{copy.source}<input value={source} onChange={event => setSource(event.target.value)} placeholder="PubMed" required /></label><label>{copy.results}<input type="number" min="0" value={count} onChange={event => setCount(event.target.value)} required /></label><button className="action" type="submit">{copy.save}</button></form>{queries && <div className="science-record-list">{queries.length ? queries.slice(0, 8).map(item => <div key={item.id}><strong>{item.query_text}</strong><span>{item.source} · {item.result_count}</span></div>) : <p>{copy.noSearch}</p>}</div>}</section>
    <section className="science-evidence-section"><div className="science-section-heading"><div><h3>{copy.claims}</h3><p>{language === "fr" ? "Une affirmation reste distincte de sa preuve et de son incertitude." : "A claim remains distinct from its evidence and uncertainty."}</p></div></div><details className="science-claim-details"><summary>{copy.addClaim}</summary><form className="science-claim-form" onSubmit={event => void submitClaim(event)}><label>{copy.type}<select value={claimType} onChange={event => setClaimType(event.target.value)}><option value="source">Source</option><option value="observation">Observation</option><option value="computed">Computed</option><option value="inference">Inference</option><option value="hypothesis">Hypothesis</option></select></label><label>{copy.claim}<input value={claimText} onChange={event => setClaimText(event.target.value)} required /></label><label>{copy.uncertainty}<input value={uncertainty} onChange={event => setUncertainty(event.target.value)} required /></label><details className="science-technical-details"><summary>{copy.detail}</summary><label>{copy.evidence}<input value={evidenceUri} onChange={event => setEvidenceUri(event.target.value)} placeholder="artifact://..." required /></label><label>{copy.selector}<input value={evidenceSelector} onChange={event => setEvidenceSelector(event.target.value)} placeholder="table:row-2" required /></label></details><button className="action" type="submit">{copy.saveClaim}</button></form></details>{claims && <div className="science-claim-list">{claims.length ? claims.map(claim => <article key={claim.id}><div><span>{claim.claim_type}</span><span>{claim.status}</span></div><strong>{claim.text}</strong><p>{claim.uncertainty}</p><div className="science-claim-actions"><button className="minor-action" type="button" onClick={() => void setClaimStatus(claim.id, "supported")}>{copy.supported}</button><button className="minor-action" type="button" onClick={() => void setClaimStatus(claim.id, "disputed")}>{copy.disputed}</button><button className="minor-action" type="button" onClick={() => void setClaimStatus(claim.id, "retracted")}>{copy.retracted}</button></div></article>) : <p>{copy.noClaim}</p>}</div>}</section>
  </section>;
}
export function ArtifactPreviewPanel({ preview }: { preview: ArtifactPreview }) {
  const label = `${preview.renderer_id}/v${preview.renderer_version}`;
  return <article className="artifact-preview" aria-label="Latest artifact preview">
    <header><div><span>PREVIEW</span><strong>{label}</strong></div><code>SHA-256 {preview.source_sha256.slice(0, 12)}</code></header>
    {preview.figure ? <div className="artifact-figure-preview"><ScientificFigureView figure={preview.figure} /></div> : preview.markdown !== undefined ? <div className="artifact-rendered-markdown"><MarkdownContent content={preview.markdown} /></div> : <div className="markdown-body artifact-rendered-html" dangerouslySetInnerHTML={{ __html: preview.html }} />}
    <footer><span>Version {preview.artifact_version_id.slice(0, 8)}</span>{preview.execution && <span>Execution: {preview.execution}</span>}{preview.resources && <span>Resources: {preview.resources}</span>}{preview.sanitizer && <span>{preview.sanitizer}</span>}</footer>
    {!!preview.warnings?.length && <p className="artifact-preview-warning">{preview.warnings.join("; ")}</p>}
  </article>;
}

export function ArtifactsSurface({ language, onInspectVersion }: { language: Language; onInspectVersion?: (versionId: string) => void }) {
  const [projectId, setProjectId] = useState("");
  const [artifacts, setArtifacts] = useState<ArtifactRecord[] | null>(null);
  const [name, setName] = useState(""); const [content, setContent] = useState(""); const [searchQuery, setSearchQuery] = useState("");
  const [searchResults, setSearchResults] = useState<Array<ArtifactRecord & { latest_version: number | null; latest_content_hash: string | null }> | null>(null);
  const [versions, setVersions] = useState<ArtifactVersion[] | null>(null);
  const [preview, setPreview] = useState<ArtifactPreview | null>(null);
  const [error, setError] = useState<string | null>(null);
  const copy = language === "fr" ? { ledger: "Registre des artefacts", artifact: "artefact versionné", description: "Les contenus sont adressés par hash. L'inspection affiche la dernière version immuable avec sa provenance.", createLabel: "Identifiant du projet, nom et contenu Markdown", project: "Identifiant du projet", result: "Nom du résultat", markdown: "Contenu Markdown", save: "Enregistrer l'artefact", searchLabel: "Recherche littérale d'artefacts", search: "Rechercher les artefacts", refresh: "Recharger les artefacts", inspect: "Inspecter l'artefact", unavailable: "Registre des artefacts indisponible.", createFailed: "Échec de la création de l'artefact.", searchFailed: "Échec de la recherche d'artefacts.", previewFailed: "Aperçu de l'artefact indisponible.", noMatches: "Aucun nom d'artefact correspondant.", latest: "dernière v", unsaved: "non enregistrée", execution: "exécution" } : { ledger: "Artifact ledger", artifact: "versioned artifact", description: "Payloads are content-addressed. Inspecting an artifact renders its latest immutable version beside its provenance.", createLabel: "Project ID, artifact name, and Markdown content", project: "Project ID", result: "Result name", markdown: "Markdown content", save: "Save artifact", searchLabel: "Literal artifact discovery", search: "Search artifacts", refresh: "Refresh artifacts", inspect: "Inspect artifact", unavailable: "Artifact ledger unavailable.", createFailed: "Artifact creation failed.", searchFailed: "Artifact search failed.", previewFailed: "Artifact preview unavailable.", noMatches: "No matching artifact names.", latest: "latest v", unsaved: "not saved", execution: "execution" };
  async function load() { try { setArtifacts((await invoke<{ artifacts: ArtifactRecord[] }>("project_artifacts_development", { projectId })).artifacts); } catch (reason) { setError(reason instanceof Error ? reason.message : copy.unavailable); } }
  async function submit(event: FormEvent) { event.preventDefault(); try { await invoke("create_project_artifact_development", { projectId, name, mediaType: "text/markdown", content }); setName(""); setContent(""); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : copy.createFailed); } }
  async function search(event: FormEvent) { event.preventDefault(); try { setError(null); setSearchResults((await invoke<{ artifacts: Array<ArtifactRecord & { latest_version: number | null; latest_content_hash: string | null }> }>("search_project_artifacts_development", { query: searchQuery, projectId: projectId || null, mediaType: null })).artifacts); } catch (reason) { setError(reason instanceof Error ? reason.message : copy.searchFailed); } }
  async function inspect(artifactId: string) { try { setError(null); const result = await invoke<{ versions: ArtifactVersion[]; preview: ArtifactPreview | null }>("project_artifact_versions_development", { artifactId }); setVersions(result.versions); setPreview(result.preview); const versionId = result.preview?.artifact_version_id ?? result.versions.at(-1)?.id; if (versionId) onInspectVersion?.(versionId); } catch (reason) { setPreview(null); setError(reason instanceof Error ? reason.message : copy.previewFailed); } }
  const title = artifacts ? (language === "fr" ? `${artifacts.length} artefact${artifacts.length === 1 ? "" : "s"} versionné${artifacts.length === 1 ? "" : "s"}` : `${artifacts.length} ${copy.artifact}${artifacts.length === 1 ? "" : "s"}`) : copy.ledger;
...
function SettingsSurface({ language: interfaceLanguage }: { language: Language }) {
  ...
  async function inspect(nextLanguage = language) {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : copy("environmentProbeUnavailable")); }
  }
  async function create(event: FormEvent) {
    event.preventDefault(); setError(null);
    try { await invoke("create_python_environment_development", { name }); setName(""); await inspect("python"); }
    catch (reason) { setError(reason instanceof Error ? reason.message : copy("environmentCreationFailed")); }
  }
  async function install(event: FormEvent) {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : copy("packageInstallationFailed")); }
  }
  const visibleEnvironments = environments.filter(environment => environment.language === language);
  useEffect(() => { void inspect(); }, []);
  return <section className="surface"><div className="surface-mark">TRUST BOUNDARY</div><h2>{copy("localEnvironments")}</h2><p>{copy("environmentDescription")}</p><form className="project-form" onSubmit={event => void create(event)}><label htmlFor="environment-name">{copy("newPythonEnvironment")}</label><input id="environment-name" value={name} onChange={event => setName(event.target.value)} placeholder="analysis" pattern="[A-Za-z0-9][A-Za-z0-9_-]{0,63}" required /><button className="action" type="submit">{copy("createPythonEnvironment")}</button></form><div className="workspace-list"><div className="workspace-row"><button className="minor-action" type="button" onClick={() => { setLanguage("python"); void inspect("python"); }}>{copy("probePython")}</button><button className="minor-action" type="button" onClick={() => { setLanguage("r"); void inspect("r"); }}>{copy("probeR")}</button></div></div>{error && <p className="agent-error">{error}</p>}{probe && <dl>{Object.entries(probe).map(([key, value]) => <><dt key={`${key}-term`}>{key}</dt><dd key={`${key}-value`}>{String(value)}</dd></>)}</dl>}{visibleEnvironments.length > 0 && <form className="project-form" onSubmit={event => void install(event)}><label htmlFor="environment-packages">{copy("environmentPackages")}</label><select value={environmentName} onChange={event => setEnvironmentName(event.target.value)} required>{visibleEnvironments.map(environment => <option key={environment.name} value={environment.name}>{environment.name}</option>)}</select><input id="environment-packages" value={packageSpecs} onChange={event => setPackageSpecs(event.target.value)} placeholder={language === "python" ? "numpy==2.0.0, pandas" : "Seurat, ggplot2"} required />{language === "r" && <><select value={channel} onChange={event => setChannel(event.target.value)}><option value="cran">CRAN</option><option value="bioconductor">Bioconductor</option></select><input value={repository} onChange={event => setRepository(event.target.value)} placeholder="https://cloud.r-project.org" required /></>}<label className="approval-check"><input type="checkbox" checked={networkApproved} onChange={event => setNetworkApproved(event.target.checked)} />{copy("approvePackageNetwork")}</label><button className="action" type="submit" disabled={!networkApproved || !packageSpecs.trim()}>{copy("installPackages")}</button></form>}{environments.length > 0 && <Evidence rows={environments.map(environment => `${environment.name}: ${environment.python_version ?? environment.runtime_version ?? environment.language}; ${environment.executable ?? copy("noExecutable")}; ${environment.package_fingerprint ?? copy("noFingerprint")}`)} />}<Evidence rows={[copy("environmentCreation"), copy("providerFallback"), copy("packageApproval")]} /></section>;
}

function ProviderSettings({ language }: { language: Language }) {
  ...
  const [result, setResult] = useState<{ provider: string; healthy: boolean; models: string[]; credential_configured: boolean; credential_handle: string | null } | null>(null);
  const [error, setError] = useState<string | null>(null);
