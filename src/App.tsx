      ...
      <main className="workspace-shell">
        ...
        <div className="workspace-grid">
          <section className="workspace-content" aria-label={language === "fr" ? activeItem.fr : activeItem.en}>
            ...
            {surface === "artifacts" && <><ArtifactsSurface language={language} /><AnnotationSurface language={language} /></>}
  ...
  return (
    <section className="science-workbench">
      ...
      <div className="science-view">
        ...
        {panel === "artifacts" && <div className="science-evidence-view"><ArtifactsSurface language={language} /><AnnotationSurface language={language} /></div>}
      </div>
    </section>
  );
}
...
export function AnnotationSurface({ language }: { language: Language }) {
  const [versionId, setVersionId] = useState(""); const [targetKind, setTargetKind] = useState("text"); const [selector, setSelector] = useState('{"offset":0}'); const [body, setBody] = useState("");
...
export function ArtifactsSurface({ language }: { language: Language }) {
  const [projectId, setProjectId] = useState("");
  ...
  async function inspect(artifactId: string) { try { setError(null); const result = await invoke<{ versions: ArtifactVersion[]; preview: ArtifactPreview | null }>("project_artifact_versions_development", { artifactId }); setVersions(result.versions); setPreview(result.preview); } catch (reason) { setPreview(null); setError(reason instanceof Error ? reason.message : copy.previewFailed); } }
