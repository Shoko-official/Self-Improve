...
export function RagSurface({ language }: { language: Language }) {
  const [uri, setUri] = useState(""); const [label, setLabel] = useState(""); const [content, setContent] = useState(""); const [selectedFile, setSelectedFile] = useState<string | null>(null); const [query, setQuery] = useState(""); const [cases, setCases] = useState('[{"query":"","expected_sources":[]}]'); const [metrics, setMetrics] = useState<{ cases: number; hits: number; recall_at_k: number } | null>(null); const [citations, setCitations] = useState<Array<{ source_label: string; source_uri: string; text: string; start_offset: number; end_offset: number }> | null>(null); const [error, setError] = useState<string | null>(null);
  async function ingest(event: FormEvent) { event.preventDefault(); try { setError(null); await invoke("rag_ingest_development", { sourceUri: uri, sourceLabel: label, content }); setContent(""); } catch (reason) { setError(reason instanceof Error ? reason.message : "FR-RAG-INGEST"); } }
  async function search(event: FormEvent) { event.preventDefault(); try { setError(null); setCitations((await invoke<{ citations: Array<{ source_label: string; source_uri: string; text: string; start_offset: number; end_offset: number }> }>("rag_search_development", { query })).citations); } catch (reason) { setError(reason instanceof Error ? reason.message : "FR-RAG-SEARCH"); } }
  async function evaluate(event: FormEvent) { event.preventDefault(); try { setError(null); setMetrics(await invoke("rag_evaluate_development", { casesJson: cases })); } catch (reason) { setError(reason instanceof Error ? reason.message : "FR-RAG-EVALUATE"); } }
