...
function ModelsSurface({ report, error, probe, engineReport, engineError, probeEngine }: { report: CapabilityReport | null; error: string | null; probe: () => Promise<void>; engineReport: EngineDoctorReport | null; engineError: string | null; probeEngine: () => Promise<void> }) { const [query, setQuery] = useState(""); const [models, setModels] = useState<HubModel[] | null>(null); const [repositoryId, setRepositoryId] = useState(""); const [filename, setFilename] = useState(""); const [destination, setDestination] = useState(""); const [sha256, setSha256] = useState(""); const [projectId, setProjectId] = useState(""); const [ollamaModel, setOllamaModel] = useState(""); const [ollamaInstall, setOllamaInstall] = useState<RuntimeInstallRecord | null>(null); const [modelError, setModelError] = useState<string | null>(null); async function search(event: FormEvent) { event.preventDefault(); try { setModelError(null); setModels((await invoke<{ models: HubModel[] }>("search_huggingface_models_development", { query, limit: 10 })).models); } catch (reason) { setModelError(reason instanceof Error ? reason.message : "Model index search failed."); } } async function download(event: FormEvent) { event.preventDefault(); try { setModelError(null); await invoke("download_huggingface_model_development", { repositoryId, filename, destination, revision: "main", expectedSha256: sha256 || null }); } catch (reason) { setModelError(reason instanceof Error ? reason.message : "Model download failed."); } } async function installOllama(event: FormEvent) { event.preventDefault(); try { setModelError(null); setOllamaInstall(await invoke<RuntimeInstallRecord>("install_ollama_model_development", { projectId, model: ollamaModel })); } catch (reason) { setModelError(reason instanceof Error ? reason.message : "Ollama install could not start."); } } return <section className="surface"><div className="surface-mark">HOST EVIDENCE</div><h2>{report ? "Host capability report" : "Capability probe unavailable"}</h2>{report ? <dl><dt>Operating system</dt><dd>{report.operatingSystem}</dd><dt>Architecture</dt><dd>{report.architecture}</dd><dt>Logical cores</dt><dd>{report.logicalCores}</dd><dt>Captured</dt><dd>{new Date(report.capturedAt * 1000).toLocaleString()}</dd></dl> : <p>{error ?? "Probing host capabilities…"}</p>}<button className="action" onClick={() => void probe()}>Refresh capability evidence</button><form className="project-form" onSubmit={event => void search(event)}><label htmlFor="hub-search">Hugging Face model index</label><input id="hub-search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search public models" required /><button className="minor-action" type="submit">Search models</button></form>{models && <Evidence rows={models.length ? models.map(model => `${model.modelId} · ${model.downloads ?? 0} downloads · ${model.likes ?? 0} likes`) : ["No public model matched this query."]} />}{models && <form className="project-form" onSubmit={event => void download(event)}><label htmlFor="hub-repository">Explicit model file transfer</label><input id="hub-repository" value={repositoryId} onChange={event => setRepositoryId(event.target.value)} placeholder="Repository ID, for example org/model" required /><input value={filename} onChange={event => setFilename(event.target.value)} placeholder="Repository filename" required /><input value={destination} onChange={event => setDestination(event.target.value)} placeholder="Local destination path" required /><input value={sha256} onChange={event => setSha256(event.target.value)} placeholder="Optional SHA-256" /><button className="action" type="submit">Download selected file</button></form>}<form className="project-form" onSubmit={event => void installOllama(event)}><label htmlFor="ollama-project">Explicit local Ollama install</label><input id="ollama-project" value={projectId} onChange={event => setProjectId(event.target.value)} placeholder="Project ID for durable install evidence" required /><input value={ollamaModel} onChange={event => setOllamaModel(event.target.value)} placeholder="Exact Ollama model, for example qwen3" required /><button className="action" type="submit">Pull selected Ollama model</button></form>{ollamaInstall && <Evidence rows={[`Install job ${ollamaInstall.id}: ${ollamaInstall.state}`, ...ollamaInstall.events.map(event => `${event.sequence_number}: ${event.detail.output ?? event.kind}`), ollamaInstall.result ? `Verified exact model: ${ollamaInstall.result.model}` : ollamaInstall.diagnostic?.code ?? "No install result recorded"]} />}{modelError && <p>{modelError}</p>}<div className="engine-report"><div className="surface-mark">DEVELOPMENT ENGINE</div><h2>{engineReport ? `Engine protocol v${engineReport.protocol_version}` : "Engine doctor unavailable"}</h2>{engineReport ? <dl><dt>Status</dt><dd>{engineReport.status}</dd><dt>Host</dt><dd>{engineReport.host.system} {engineReport.host.release}, {engineReport.host.machine}</dd><dt>Checked</dt><dd>{new Date(engineReport.checked_at).toLocaleString()}</dd></dl> : <p>{engineError ?? "Probing development engine…"}</p>}<button className="action" onClick={() => void probeEngine()}>Refresh engine doctor</button><p className="development-note">Development-only IPC. A download registers an unvalidated local file; it does not enable a runtime.</p></div><Evidence rows={["No inference runtime pack installed", "No model file imported", "No remote provider has received data"]} /></section>; }
