...
`frontierctl create-r-environment --name analysis` creates a project-scoped R library only when `Rscript` is available and records its exact runtime. `frontierctl install-r-packages --name analysis --package rjson` uses an explicit HTTPS CRAN repository and records installed package versions plus a deterministic fingerprint. Missing R returns `FR-ENV-R-NOT-FOUND`; Frontier never substitutes Python. Jupyter integration remains separate work.
