...
| Hugging Face model registry | partially_verified | Explicit public-index search, dry-run planning, optional `hf_xet`, bounded parallel ranges, cancellation, resume, atomic writes, streaming SHA-256 verification, and post-transfer local registration are covered in `engine/tests/test_model_registry.py`; a downloaded file remains runtime-unvalidated |
