...
`application/vnd.shokos.figure+json` version 1 supports five non-executing figure kinds:
...
- `tree` accepts a bounded rooted hierarchy with unique node IDs, bounded labels, and exact node selectors.
- `genome` accepts bounded genomic regions and coordinate-checked features with unique IDs and exact feature selectors.
...
Molecular, interactive three-dimensional structure, PDF, and publication export renderers remain unavailable. They must be added with representative fixtures, malformed and large-file tests, accessibility checks, and structural or pixel assertions before being shown in the product.
